...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(
        ...
        "saved_hangs, max_depth, execs_per_sec, total_execs, edges_found, unique_diffs\n");
...
void check_binary(afl_state_t *afl, u8 *fname) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
    ...
    for (u32 i = 0; i < afl->diff_num; i++) {
      // zero for native binary
      if(i == 0){
        afl->diff_fsrv[i].target_path = alloc_printf("%s-%d", fname, i);
      }
      // one for wasm
      else if(i == 1){
        afl->diff_fsrv[i].target_path = getenv("AFL_WASM_RUNTIME");
        OKF("Successfully get the Webassembly runtime:%s",afl->diff_fsrv[i].target_path);
      }
      if (stat(afl->diff_fsrv[i].target_path, &st) || !S_ISREG(st.st_mode) ||
          !(st.st_mode & 0111) || (f_len_diff = st.st_size) < 4) {
        FATAL("Program '%s' not found or not executable", afl->diff_fsrv[i].target_path);
      }
    }
