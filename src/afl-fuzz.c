...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+a:Ab:y:J:B:c:CdDe:E:f:F:g:G:hi:I:l:L:m:M:nNo:Op:P:QRs:S:t:"
                       "T:UV:WXx:YZ")) > 0) {

    switch (opt) {

      case 'y':
        if (sscanf(optarg, "%u", &(afl->diff_num)) < 1 || optarg[0] == '-') FATAL("Invalid number of compilers: Bad syntax used for -y");
        break;
  ...
  // insurance
  if(afl->diff_num != 2){
    FATAL("You specified more than 2 diff forkservers!\nPlease change the afl-fuzz-init.c!\n");
  }
  ...
  for (u32 i = 0; i < afl->diff_num; i++) {
    afl->diff_fsrv[i].child_kill_signal = afl->fsrv.child_kill_signal;
    afl->diff_fsrv[i].fsrv_kill_signal = afl->fsrv.fsrv_kill_signal;
  }
  ...
  //wx
  wasm_argv = use_argv;
  int len_wasm_argv = argc - optind;

  // wx
  for (u32 i = 0; i < afl->diff_num; i++) {
    afl->diff_fsrv[i].trace_bits = afl_shm_init(
        &afl->shm, afl->diff_fsrv[i].map_size, afl->non_instrumented_mode);
  }
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    for (u32 i = 0; i < afl->diff_num; i++) {
      if(i == 0){
        afl_fsrv_start_diff(&afl->diff_fsrv[i], i, afl->argv, &afl->stop_soon,
                          afl->afl_env.afl_debug_child);
      }

      else if(i == 1){

        // Handling of wasm parameters
        
        //wasm_argv[len_wasm_argv+2] = NULL;
        //for(int i = len_wasm_argv+1;i -2 > 0;i-- ){
        //  wasm_argv[i] = alloc_printf("%s",wasm_argv[i-2]);
        //}
        //wasm_argv[1] = alloc_printf("--dir=./ run");
        //wasm_argv[2] = alloc_printf("%s.wasm", wasm_argv[0]);
        //// get the wasm runtime path
        //char * wavm_path = getenv("AFL_WASM_RUNTIME");
        //if(!wavm_path){
        //  FATAL("Environment variable AFL_WASM_RUNTIME needed!");
        //}
